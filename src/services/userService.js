...
/**
 * Servicio para gestión de usuarios del dashboard
 */
export default {
  /**
  ...
  authenticateUser: async (username, password) => {
    ...
    try {
      await this.updateLastAccess(user.UsuarioID);
    } catch (err) {
   ...
   */
  createUser: async ({ username, password, rol, nombre, email, creadoPor }) => {
    // Validar rol
   ...
   * @returns {Promise<void>}
   */
  logAccess: async (userId, ip, exitoso, detalles = null) => {
    try {
      const pool = await getPool();
      await pool.request()
        .input('userId', sql.Int, userId)
